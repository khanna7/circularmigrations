...
mig.vital.dynamics <- function(net,
  ...
  cat("Number of new deaths is ", length(new.dying), ".", 
   ...
   newpop <- activate.vertices(newpop,
    ...
    if (length(dying)>0) {
      edges.with.dying.node <- sort(unique(unlist(sapply(dying, function(x)get.edgeIDs.active(newpop, at=curr.time, v=x)))))
    } else {
...
#    active.edgeIDs <- (1:nedge.cum)[is.active(newpop,prev.time, e=(1:nedge.cum))]
                                        # XXXXXXXXXXXX
    active.edgeIDs <- (1:nedge.cum)[is.active(newpop,
                                              at=curr.time,
                                              e=(1:nedge.cum))]

     surviving.edges <- setdiff(active.edgeIDs,edges.with.dying.node)
     newpop <- activate.edges(newpop, onset=curr.time,
                              terminus=(curr.time+1),
                              e=surviving.edges
  )
  ...
  if (nbirths>0){
    ...
    newpop <- activate.vertices(newpop,onset=curr.time,
                                terminus=curr.time+1,v=birthIDs)
