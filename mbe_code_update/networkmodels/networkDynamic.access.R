...
activate.edges<-function(x, onset=-Inf, terminus=NULL, e=seq_along(x$mel)){
...
activate.vertices<-function(x, onset=-Inf, terminus=NULL, v=seq_len(network.size(x))){
  #Get existing activity attributes
...
#the appropriate mark.  Edges without an "active" attribute are given one.
deactivate.edges<-function(x, onset=Inf, terminus=NULL, e=seq_along(x$mel)){
  e<-e[!sapply(x$mel[e],is.null)]  #Filter out non-edges
  if(length(e)==0)
    return()
  #Get existing activity attributes
...
#one.
deactivate.vertices<-function(x, onset=Inf, terminus=NULL, v=seq_len(network.size(x))){
  #Get existing activity attributes
  active<-lapply(x$val[v],"[[","active")
  #print(active)
  #Update as needed
...
network.dyadcount.active<-function (x, onset, terminus=NULL, rule=c("any","all"), na.omit = TRUE, active.default=TRUE) 
{
    ...
    if (is.directed(x)) {
        ...
        if (is.bipartite(x)) {
            nactor <- sum(is.active(x=x,onset=onset,terminus=terminus,e=NULL, v=seq_len(get.network.attribute(x, "bipartite")),rule=rule, active.default=active.default))
...
network.edgecount.active<-function (x, onset, terminus=NULL, rule=c("any","all"), na.omit = TRUE, active.default=TRUE){
  act<-is.active(x=x,onset=onset,terminus=terminus,e=seq_len(x%n%"mnext"-1), v=NULL,rule=rule, active.default=active.default)
...
network.size.active<-function(x,onset,terminus=NULL,rule=c("any","all"),active.default=TRUE){
  sum(is.active(x=x,onset=onset,terminus=terminus,e=NULL,v=seq_len(network.size(x)), rule=rule,active.default=active.default))
}
