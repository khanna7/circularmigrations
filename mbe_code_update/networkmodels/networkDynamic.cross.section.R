...
network.crosssection<-function(x,onset,terminus=NULL,rule=c("any","all"),active.default=TRUE){
  valv<-is.active(x=x,onset=onset,terminus=terminus,e=NULL,v=seq_len(network.size(x)), rule=rule, active.default=active.default)
  vale<-is.active(x=x,onset=onset,terminus=terminus,e=seq_along(x$mel),v=NULL, rule=rule, active.default=active.default)
  #Create network
  n<-sum(valv)
  if(n==0)
    return(list())
  ...
  if(net%n%"bipartite">0)
    net%n%"bipartite"<-sum(is.active(x=x,onset=onset,terminus=terminus,e=NULL, v=seq_len(x%n%"bipartite"),rule=rule, active.default=active.default))
